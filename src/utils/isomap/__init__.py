...
def make_adjacency(data, dist_func="euclidean", eps=1, return_predecessors=False):
    """
   ...
   """
    ...
    bln = dist < eps
    adj[bln] = dist[bln]
    short = shortest_path(adj, return_predecessors=return_predecessors)
    return short


def get_path(predecessors, distances, i, j):
    path = [int(j)]
    path_length = []
    k = int(j)
    while int(predecessors[i, k]) != -9999:
        path.append(int(predecessors[i, k]))
        path_length.append(distances[i, k])
        k = int(predecessors[i, k])
